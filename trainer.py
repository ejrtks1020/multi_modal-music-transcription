...
class Solver(object):
    ...
    def __init__(self, data_loader, test_data_loader, model, criterion, optimizer, lr_scheduler, epochs):
        self.save_model_path = './weights/custom_lstm.pth' # change to your path
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            start = time.time()
            # training loop
            tr_avg_loss, tr_avg_precision, tr_avg_recall, tr_avg_f1 = self.train_loop()
            ...
            print('Train Summary | Epoch {0} | Time {1:.2f}s | '
                  'Train Loss {2:.3f} | precision {3:.3f} | recall {4:.3f} | f1_score {5:.3f}'.format(
                epoch+1, time.time() - start, tr_avg_loss, tr_avg_precision, tr_avg_recall, tr_avg_f1))
            ...
            torch.save(self.net.state_dict(), "./weights/custom_lstm_last.pth")
    ...
    def train_loop(self):
        ...
        for i, data in enumerate(data_loader):
            imgs, audio, label = data[0].float().cuda(), data[1].float().cuda(), data[2].float().cuda()
            logits = self.net(imgs, audio)
            ...
            precision_re = metrics.precision_score(numpy_label, numpy_pre_label, average=None, zero_division=0)
            recall_re = metrics.recall_score(numpy_label, numpy_pre_label, average=None, zero_division=0)
            f1_score_re = metrics.f1_score(numpy_label, numpy_pre_label, average=None, zero_division=0)

            precision = np.nansum(precision_re) / np.count_nonzero(~np.isnan(precision_re))
            recall = np.nansum(recall_re) / np.count_nonzero(~np.isnan(recall_re))
            f1_score = np.nansum(f1_score_re) / np.count_nonzero(~np.isnan(f1_score_re))
            if self.global_step % 100 == 0:
                end = time.time()
                print(
                    "step {0} loss:{1:.4f} | precision:{2:.3f} | recall:{3:.3f} | f1-score:{4:.3f} | time:{5:.2f}".format(self.global_step, loss.item(), precision,
                                                                                        recall, f1_score, end - start))
                start = end
        ...
        return epoch_loss/count, epoch_precision/count, epoch_recall/count, epoch_f1/count
    ...
    def validate(self):
        ...
        # all_precision = metrics.precision_score(all_label, all_pred_label, average="micro", zero_division=0)
        # all_recall = metrics.recall_score(all_label, all_pred_label, average="micro", zero_division=0)
        # all_f1_score = metrics.f1_score(all_label, all_pred_label, average="micro", zero_division=0)
        precision_re = metrics.precision_score(all_label, all_pred_label, average=None, zero_division=0)
        recall_re = metrics.recall_score(all_label, all_pred_label, average=None, zero_division=0)
        f1_score_re = metrics.f1_score(all_label, all_pred_label, average=None, zero_division=0)

        all_precision = np.nansum(precision_re) / np.count_nonzero(~np.isnan(precision_re))
        all_recall = np.nansum(recall_re) / np.count_nonzero(~np.isnan(recall_re))
        all_f1_score = np.nansum(f1_score_re) / np.count_nonzero(~np.isnan(f1_score_re))        
